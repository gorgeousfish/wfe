...
{
    real matrix Phi_use, V_eig
    real rowvector phi_evals, lambda_eig
    ...
    // BUG-001 fix: clamp tiny negative eigenvalues to zero so that
    // pinv(Phi) is exactly PSD and the quadratic form stays >= 0.
    if (min(phi_evals) < 0) {
        symeigensystem(Phi_use, V_eig, lambda_eig)
        lambda_eig = lambda_eig :* (lambda_eig :> 0)
        Phi_use = V_eig * diag(lambda_eig) * V_eig'
        Phi_use = 0.5 :* (Phi_use + Phi_use')
    }
